...
const Inventory = () => {
  ...
  const handleEditItem = (item) => {
    ...
    const formattedDate = item.expiryDate && item.expiryDate !== "Not Specified" 
  ...
  const validateForm = () => {
    ...
    // Expiry Date validation - only validate if a date is provided
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        expiryDate: formData.expiryDate || "Not Specified" // Set as "Not Specified" if empty
      };
  ...
  const generatePDF = () => {
    ...
    const data = inventoryItems.map(item => ({
      ...
      expiryDate: item.expiryDate === "Not Specified" ? "Not Specified" : new Date(item.expiryDate).toLocaleDateString()
    }));
  ...
  const formatDate = (dateString) => {
    if (dateString === "Not Specified") return "Not Specified";
  ...
  const isExpiringSoon = (expiryDate) => {
    if (expiryDate === "Not Specified") return false;
          ...
          <Row>
            ...
            <Col md="6">
              <FormGroup>
                ...
                <small className="form-text text-muted">
                  Leave blank if not applicable - will be saved as "Not Specified"
                </small>
              </FormGroup>
            </Col>
          </Row>
